...
(deftest cidv1-raw-matches-ipfs
  (is (= "bafkreihdwdcefgh4dqkjv67uzcmw7ojee6xedzdetojuzjevtenxquvyku"
         (mf/cidv1-raw (byte-array 0))) "empty input")
  ...
  (is (= "bafkreifsjhh4xb4ct3q652hbcbayexs57mu46imyrjxua4r6ofgft3qmv4"
         (mf/cidv1-raw (.getBytes "multiformats-clj" "UTF-8")))))

;; ── multihash framing ─────────────────────────────────────────────────────────
(deftest multihash-sha256-frames-0x12-0x20
  (let [mh (mf/multihash-sha256 (byte-array 0))]
    (is (= 34 (count mh)))
    ...
    (is (= "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
           (mf/hexify (byte-array (drop 2 (seq mh))))))))
...
(deftest base58-roundtrip
  (doseq [s ["" "Satoshi" "the quick brown fox"]]
    (let [b (.getBytes s "UTF-8")]
      (is (= (seq b) (seq (mf/base58btc-decode (mf/base58btc b)))))))
  ;; leading zero bytes → leading '1's (33 → alphabet index 33 = 'a')
  (is (= "11a" (mf/base58btc (byte-array [0 0 (unchecked-byte 33)]))))
  (is (= (seq (byte-array [0 0 (unchecked-byte 33)]))
         (seq (mf/base58btc-decode "11a"))))
  (is (= [(byte 0)] (seq (mf/base58btc-decode "1"))) "'1' decodes to a single zero byte"))

;; ── base32 round-trip ─────────────────────────────────────────────────────────
(deftest base32-roundtrip
  (doseq [s ["" "f" "fo" "foo" "foob" "multiformats"]]
    (let [b (.getBytes s "UTF-8")]
      (is (= (seq b) (seq (mf/base32-decode (mf/base32 b))))))))

;; ── CID decode round-trips its bytes ──────────────────────────────────────────
(deftest cid-bytes-roundtrip
  (let [c (mf/cidv1-raw (.getBytes "round-trip" "UTF-8"))]
...
(deftest kotoba-cid-is-dag-cbor
  (let [c (mf/kotoba-cid "ibuki")]
    (is (clojure.string/starts-with? c "bafyrei"))
